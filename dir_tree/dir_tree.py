...
__all__ = ['create_sub_dirs', 'get_dirs_files', 'tree']
...
# Cell
def create_sub_dirs(path, num_dirs, sub_dir_name='sub_dir', levels=1):
    for i in range(num_dirs):
        sub_dir = path/(sub_dir_name + f"_{i+1}")
        sub_dir.mkdir(exist_ok=True)
        if levels: create_sub_dirs(sub_dir, i+1+num_dirs, sub_dir.name, levels-1)
